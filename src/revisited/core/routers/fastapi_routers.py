...
def add_fastapi_router(app: FastAPI, router: APIRouter) -> FastAPI:
    app.include_router(router)
    return app


def __walk(directory: Path) -> Generator[Tuple[str, list[str], list[str]], None, None]:
    """Walk through the directory yielding each folder's path and filenames."""
    yield from os.walk(directory)


def __square_to_curly_brackets(path: str) -> str:
...
#     """
#     start = time()
#     logging.log(f"Loading routes from {directory}", verbose)
#
#     routers: list[Tuple[str, APIRouter]] = []
#
#     for root, _, files in __walk(directory):
#         root = Path(root)
