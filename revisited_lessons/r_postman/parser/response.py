...
from ..errors import RequestParserException, ResponseParserException
from ..utils import is_response, map_to_dict, strip_charset
...
class Response:
    def __init__(self, response=None):
        if (response is None) or (
            not isinstance(response, dict) or (not is_response(response))
        ):
    ...
    def get_body(self):
        responsebody = self.response["body"] if "body" in self.response else None
        try:
