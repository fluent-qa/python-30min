...
from ..utils import filter_env_var, format_path, is_request, map_to_dict
...
class Request:
    def __init__(self, request=None):
        if (request is None) or (
            not isinstance(request, dict) or (not is_request(request))
        ):
    ...
    def get_body(self):
        if "body" in self.request:
            ...
            if self.body_contenttype in ["application/json"] and isinstance(
                bodyitem, str
            ):
